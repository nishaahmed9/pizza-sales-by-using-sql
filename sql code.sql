...
-- Calculate the total revenue generated from pizza sales.  ctrl + B for beautify
SELECT 
...
select quantity, count(order_details_id) as most_cpo
...
-- rank() over(partition by category, order by revenue, esc) as rn 
-- from
